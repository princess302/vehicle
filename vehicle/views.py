...
def admin_add_vehicle_view(request):
    ...
    return render(request, 'vehicle/admin_add_vehicle.html', {'vehicle_form': vehicle_form})

@login_required(login_url='adminlogin')
def admin_add_route_view(request):
    route_form = forms.RouteForm()
    if request.method == 'POST':
        route_form = forms.RouteForm(request.POST)
        if route_form.is_valid():
            route_form.save()
            return HttpResponseRedirect('admin-dashboard')
    return render(request, 'vehicle/admin_add_route.html', {'route_form': route_form})
...
@login_required(login_url='adminlogin')
def admin_view_routes_view(request):
    routes = models.Route.objects.all()
    return render(request, 'vehicle/admin_view_routes.html', {'routes': routes})
